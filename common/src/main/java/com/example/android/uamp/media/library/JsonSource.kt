 ...
 */
class JsonSource(private val applicatContext: Context, private val source: Uri) :
    AbstractMusicSource() {
        ...
        return withContext(Dispatchers.IO) {
            ...
            val mediaMetadataCompats = musicCat.music.map { song ->
                MediaMetadataCompat.Builder()
                    .from(song)
                    .apply {
                        val uri = Glide.with(applicatContext).downloadOnly().load(song.image).submit().get().asAlbumArtContentUri()
                        displayIconUri = uri.toString() // Used by ExoPlayer and Notification
 ...
 * that if the JSON was at "https://www.example.com/json/music.json" then the  would be found
